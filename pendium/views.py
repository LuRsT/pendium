...
def index():
    p = g.wiki.root()
    home_file = get_home()
    return render_template("index.html", file=p, files=p.items(), home_file=home_file)
...
def edit(path):
    ...
    return render_template(
        "edit.html", file=p, files=p.items(), file_content=escape(p.content())
    )
...
def view(path, ref=None):
    ...
    if p.is_leaf and not p.is_binary:
        ...
        return render_template(
            "view.html", file=p, files=p.items(), refs=p.refs, rendered=p.render()
        )
    elif p.is_node:
        home_file = get_home(p.path)
        return render_template(
            "list.html", file=p, files=p.items(), home_file=home_file
        )
    else:
...
def search():
    ...
    if request.args.get("q", None):
        ...
        js = json_dumps(
            {"searched": True, "term": term, "hits": len(hits), "results": hits_dicts,}
        )
